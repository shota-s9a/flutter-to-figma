...
- **`use_figma` ツールの使用は禁止。** Figma への反映は必ず Code to Canvas（`generate_figma_design`）で行う。`use_figma` や Figma MCP のデザイン生成系ツール（`generate_figma_design` 以外）も使用しない。
- **各 Step の開始時に TaskCreate、完了時に TaskUpdate(completed) を実行すること。** 前の Step が completed になるまで次の Step に進んではならない。Step 内で問題が発生した場合はユーザーに報告し、指示を待つ（自己判断でスキップしない）。
...
> TaskCreate: "Step 1: 情報収集（Flutter エクスポート + スクリーンショット）"
...
**完了条件:** JSON ファイルが生成され、スクリーンショットが1枚以上取得できていること
> TaskUpdate → completed

### 2. 実装コードを読む（最重要ステップ）

> TaskCreate: "Step 2: 実装コード精読"
...
**完了条件:** 対象画面の Widget ツリー・画像/アイコン一覧・テーマトークン解決がすべて完了していること
> TaskUpdate → completed

### 3. HTML/CSS を書く

> TaskCreate: "Step 3: HTML/CSS 作成"
...
**完了条件:** index.html が作成されていること
> TaskUpdate → completed

### 4. ローカルで配信する

> TaskCreate: "Step 4: HTML 配信"

`serve_html` に上記ディレクトリを渡して URL を得る。

**完了条件:** localhost の URL が取得できていること
> TaskUpdate → completed

### 5. Playwright でレンダリング結果を撮る

> TaskCreate: "Step 5: Playwright スクリーンショット"
...
**完了条件:** Playwright のスクリーンショットが保存されていること
> TaskUpdate → completed

### 6. シミュレータ画像と差分チェック

> TaskCreate: "Step 6: 差分チェック（最大3回リトライ）"
...
**完了条件:** 差分が許容範囲内、またはリトライ上限に達してユーザーに報告済み
> TaskUpdate → completed

### 7. ユーザーに品質確認

> TaskCreate: "Step 7: ユーザー品質確認"

URL と Playwright スクショをユーザーに提示して、「この状態で Figma に送っていいか」を必ず確認する。**勝手に Figma に送らない**。

**完了条件:** ユーザーから OK をもらっていること
> TaskUpdate → completed

### 8. Figma に送る（Code to Canvas）

> TaskCreate: "Step 8: Code to Canvas で Figma に送信"

ユーザー OK が出たら、**`generate_figma_design`（Code to Canvas）** を呼ぶ。**⚠️ `use_figma` は使わない。必ず `generate_figma_design` を使う。**

- 初回は `outputMode: "newFile"` で新規ファイル作成
- `captureId` が返ってきたら、最大 10 回まで polling して完了を待つ
- 完了後、Figma のファイル URL をユーザーに返す

**完了条件:** Figma ファイル URL がユーザーに共有されていること
> TaskUpdate → completed

### 9. 後片付け

> TaskCreate: "Step 9: 後片付け"

`stop_html_server` で HTTP サーバーを停止する。一時ディレクトリは、ユーザーが再利用したいかもしれないので自動削除しない。

**完了条件:** HTTP サーバーが停止していること
> TaskUpdate → completed
