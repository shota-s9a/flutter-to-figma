 ...
 */
export function extractDecorationsFromProject(projectRoot, themeTokens) {
    ...
    for (const file of dartFiles) {
        ...
        const classMatches = content.matchAll(/class\s+(_?\w+)\s+extends\s+\w*Widget\b/g);
        for (const classMatch of classMatches) {
            ...
            const gradMatch = classBody.match(/LinearGradient\s*\(([\s\S]*?)\)\s*[,)]/);
            if (gradMatch) {
                const gradient = parseLinearGradientFromSource(gradMatch[0], themeTokens);
                if (gradient)
                    dec.gradient = gradient;
            }
            // Parse BoxShadow
            ...
            const radiusMatch = classBody.match(/BorderRadius\.circular\s*\(\s*(?:([0-9.]+)|context\.radius\.(\w+)|radius\.(\w+))\s*\)/);
            if (radiusMatch) {
                if (radiusMatch[1])
                    dec.borderRadius = parseFloat(radiusMatch[1]);
                else if (themeTokens) {
...
function parseLinearGradientFromSource(src, tokens) {
    const colorRefs = [];
    ...
    if (tokens) {
        ...
        for (const m of tokenRefs) {
            ...
            if (resolved)
                colorRefs.push(resolved);
        }
    }
    if (colorRefs.length < 2)
        return null;
...
function parseBoxShadowFromSource(src, tokens) {
    let color = null;
...
function parseBorderFromSource(src, tokens) {
    let color = null;
    const direct = src.match(/Color\(0x([0-9a-fA-F]{8})\)/);
    if (direct) {
        const hex = direct[1];
...
export function extractThemeTokens(projectRoot) {
    ...
    for (const file of dartFiles) {
        ...
        const classMatches = [
            ...content.matchAll(/class\s+(\w+)(?:\s+extends[^{]*)?\s*\{/g),
        ];
        for (const classMatch of classMatches) {
            const className = classMatch[1];
            ...
            const screenUtilMatches = classBody.matchAll(/(\w+):\s*([0-9.]+)\.[whrsp]+,/g);
            // `static const double name = N;` / `final double name = N;`
            const constDoubleMatches = classBody.matchAll(/(?:static\s+const|final|const)\s+(?:double|int)\s+(\w+)\s*=\s*([0-9.]+)/g);
            // `double get name => N;`  / `double get name => N.w;`
            const getterMatches = classBody.matchAll(/(?:double|int)\s+get\s+(\w+)\s*=>\s*([0-9.]+)(?:\.[whrsp]+)?/g);
...
function addColor(tokens, name, hex) {
    if (tokens.colors[name])
        return; // first occurrence wins (light theme)
    tokens.colors[name] = {
        a: parseInt(hex.substring(0, 2), 16) / 255,
        r: parseInt(hex.substring(2, 4), 16) / 255,
        g: parseInt(hex.substring(4, 6), 16) / 255,
...
export function parseLinearGradient(desc) {
    if (!desc.includes("LinearGradient"))
        return null;
    // Extract Color list
...
export function parseBoxShadow(desc) {
    if (!desc.includes("BoxShadow"))
        return null;
...
export function parseBorder(desc) {
    if (!desc.includes("Border") && !desc.includes("BorderSide"))
        return null;
    const colorMatch = desc.match(/Color\(0x([0-9a-fA-F]{8})\)/);
    if (!colorMatch)
        return null;
...
export function resolveIconName(iconDataDesc) {
...
export class FigmaConverter {
    ...
    constructor(inspector, options) {
        ...
        if (options?.projectRoot) {
            ...
            this.decorations = extractDecorationsFromProject(options.projectRoot, this.themeTokens);
        }
        else {
    ...
    async convertNode(node) {
        ...
        if (userClass)
            this.ancestorStack.push(userClass);
        // Get detailed info if objectId is available
        let props = [];
        let layout = null;
        if (node.objectId) {
            try {
                const details = await this.inspector.getDetailsSubtree(node.objectId, 1);
        ...
        const figmaNode = this.buildFigmaNode(widgetType, node, props, size, offset, children);
        // Pop ancestor stack
        if (userClass)
    ...
    buildFigmaNode(widgetType, node, props, size, offset, children) {
        const base = {
            x: offset.offsetX,
            y: offset.offsetY,
            width: size.width,
        ...
        if (widgetType === "Text" || widgetType === "RichText") {
            ...
            if (styleProp?.description) {
                ...
                if (weightMatch)
                    textNode.fontWeight = weightMatch;
                const sizeMatch = styleProp.description.match(/size:\s*([0-9.]+)/);
                if (sizeMatch)
                    textNode.fontSize = parseFloat(sizeMatch[1]);
            }
            return textNode;
        }
        ...
        if (widgetType === "Container" ||
                ...
                figma.fills = [
                    {
                        ...
                        gradientStops: sourceDec.gradient.colors.map((c, i) => ({
                            color: c,
                            position: sourceDec.gradient.stops[i] ??
            ...
            for (const prop of props) {
                ...
                if ((prop.name === "decoration" || prop.name === "gradient") &&
                ...
                // BoxShadow
                if ((prop.name === "decoration" || prop.name === "boxShadow") &&
        ...
        if (widgetType === "Icon") {
            const iconDataProp = props.find((p) => p.description?.includes("IconData"));
