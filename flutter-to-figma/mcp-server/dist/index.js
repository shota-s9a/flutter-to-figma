...
server.tool("serve_html", "指定ディレクトリを静的 HTTP サーバーとして配信する。Figma の Code to Canvas (generate_figma_design) に食わせる HTML プレビュー用。返り値の URL をそのまま Playwright / generate_figma_design に渡せる。サーバーは stop_html_server で停止するまで動き続ける。", {
    directory: z
        .string()
...
server.tool("stop_html_server", "serve_html で起動したローカル HTTP サーバーを停止する", {
